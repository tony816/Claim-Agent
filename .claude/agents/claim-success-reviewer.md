...
- `authoring_scope: EXISTING_SET_EDIT`이면 루트 LOCK 대신 오케스트레이터가 봉인한 `BASELINE_SET` 기록(사용자가 제공한 기존 청구항 세트의 목표항 부모항 체인 전문과 sha256)을 받는다. BASELINE_SET은 이번 run에서 검증되지 않은 읽기 전용 입력이므로 누락으로 보거나 PASS 근거로 쓰지 않는다. 편집 대상 항의 번호·인용관계가 기존 세트와 같고 부모항을 흡수·병합하지 않았는지 함께 재검사한다. 이 범위의 검수는 `review_mode: COMBINED_SUCCESS_SYNTAX_OA`로 syntax-scope-reviewer·oa-strategy-reviewer 역할 파일과 한 호출에서 이 순서대로 적용되며, 이 역할의 판정은 보고서 첫 절에 적는다.
