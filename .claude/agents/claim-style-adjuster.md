...
작업 시작 시 `sources/07_용어표현_출처게이트.md`와 `sources/04_청구항_스타일가이드.md` 전문을 읽는다. 의미 초안과 설계 계약에 특정된 기술내용만 사용하며, 두 후처리 문서나 역사 코퍼스를 발명의 기술적 근거로 사용하지 않는다.

코퍼스 또는 라우팅 인덱스는 다음 두 경우에만 제한적으로 사용할 수 있다.
...
검색한 경우 파일, 예시 ID 또는 청구항, 정확한 조각, 해결 대상, 재검증 결과와 범위 불변 결과를 모두 기록한다. 적합한 조각이 없으면 `예시 없음`으로 진행한다.
