...
# Claim-Agent Web — 격리 blind 대화 프롬프트

이 프롬프트는 원래 Claim-Agent 프로젝트가 연결되지 않은 새 빈 대화에서만 사용한다. 프로젝트 파일, 이전 대화, 작성자의 설명 또는 기준 도면이 자동으로 보이는 대화에서는 사용하지 않는다. 종속항은 목표항마다 서로 다른 새 빈 대화를 사용한다.
