...
5. 출력은 제공된 JSON 스키마를 따르는 JSON 객체 하나뿐이다. `report_markdown`에는 역할 파일의 `출력 형식` 항목을 빠짐없이 한국어로 다루되 **압축형**으로 쓴다: 항목마다 판정값과 결정적 사유를 1~2문장으로 적고, 표는 판정에 쓰인 행(근거·결함·REVIEW 사유가 있는 행)만 싣는다. 입력으로 받은 청구항·원자료·상위 보고서 내용을 다시 옮겨 적지 않고 record_id·항목 번호·위치로 참조한다. 이 역할이 새로 작성하거나 수정한 청구항 문언과 조건 4 근거표는 생략하지 않으며, 모든 게이트는 `게이트명: 값` 줄로 적는다. 목표 분량은 3,000자 이내다. `status`, `gates`, `next_step`, `handoff_ready`, `exact_claim_text`는 `report_markdown`의 내용과 글자 단위로 일치해야 한다. 특히 `exact_claim_text`에는 청구항 전문을 줄바꿈·문장부호까지 그대로 넣는다.
